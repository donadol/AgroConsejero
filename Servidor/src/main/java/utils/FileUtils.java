	...
	public static List<Informacion> leerInformacion(List<Topico>topicos){
		...
		try {
			...
			for(int i = 0; i<infoJson.length(); ++i) {
				...
				aux = new Informacion(Zona.valueOf(jsonObj.getString("zona")), jsonObj.getString("titulo"), jsonObj.getString("descripcion"), Integer.parseInt(jsonObj.getString("tiempo")), null);
